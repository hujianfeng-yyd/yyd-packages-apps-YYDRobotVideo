...
/**
 * Replacement for WebSocketChannelClient for direct communication between two
 * IP addresses. Handles the signaling between the two clients using a TCP
 * connection.
 *
 * <p>
...
public class TCPChannelClient {
	...
	/**
	 * Callback interface for messages delivered on TCP Connection. All
	 * callbacks are invoked from the looper executor thread.
	 */
	 ...
	 *
	 * @param eventListener
	 *            Listener that will receive events from the client.
	 * @param ip
	 *            IP address to listen on or connect to.
	 * @param port
	...
	/**
	 * Helper method for firing onTCPError events. Calls onTCPError on the
	 * executor thread.
	 */
	...
	/**
	 * Base class for server and client sockets. Contains a listening thread
	 * that will call eventListener.onTCPMessage on new messages.
	 */
	private abstract class TCPSocket extends Thread {
		...
		/**
		 * Closes the rawSocket if it is still open. Also fires the onTCPClose
		 * event.
		 */
		public void disconnect() {
			try {
		...
		/**
		 * Sends a message on the socket. Should only be called on the executor
		 * thread.
		 */
		public void send(String message) {
