...
/**
 * Implementation of AppRTCClient that uses direct TCP connection as the
 * signaling channel. This eliminates the need for an external server. This
 * class does not support loopback connections.
 */
public class DirectRTCClient implements AppRTCClient, TCPChannelClient.TCPChannelEvents {
	private static final String TAG = DirectRTCClient.class.getSimpleName();
	...
	public void onTCPConnected(boolean isServer) {
		if (isServer) {
			...
			SignalingParameters parameters = new SignalingParameters(
					...
					new LinkedList<PeerConnection.IceServer>(), isServer, // Server side acts as the initiator on direct connections.
					null, // clientId
					null, // wssUrl
					null, // wwsPostUrl
					null, // offerSdp
	...
	public void onTCPMessage(String msg) {
		try {
			...
			if (type.equals("candidate")) {
				...
				SignalingParameters parameters = new SignalingParameters(
						// Ice servers are not needed for direct connections.
						new LinkedList<PeerConnection.IceServer>(), false, // This code will only be run on the client side. So, we are not the initiator.
						null, // clientId
						null, // wssUrl
						null, // wssPostUrl
						sdp, // offerSdp
