...
public class ActivityDialByNumber extends BaseActivity {
	...
	protected void onResume() {
		log.d(TAG, "onResume()");
		super.onResume();
		if (WebRtcSDKHelper.getInstance().haveNewHistory()) {
			log.d(TAG, "haveNewHistory()");
			initHistoryList();
		}
		else {
			log.d(TAG, "Not haveNewHistory()");
		}
	}
	
	private void initHistoryList() {
		mCallHistoryList = YYDVideoServer.getInstance().getCallHistoryList();
		...
		WebRtcSDKHelper.getInstance().setHaveNewHistory(false);
	}
	...
	private OnListItemClickListener mOnListItemClickListener = new OnListItemClickListener() {
		public void onClick(int position, CallHistory history) {
			...
			if (user != null) {
				meetingInvite(user.getNumberType(), user.getCallNumber(),
	...
	private OnClickListener dialClick = new OnClickListener() {
		...
		public void onClick(View view) {
			...
			if (strNumber.length() > 0) {
				...
				meetingInvite(strNumber.length() >= 11 ? NumberType.Phone : NumberType.Robot, strNumber, strNumber);
			}
		}
	};
	...
	public void meetingInvite(final String numberType, final String callNumber, final String callName) {
		...
		if (callNumber.equals(YYDVideoServer.getInstance().getLoginUser().getCallNumber())) {
		...
		dialog.setListener(new InviteDialog.Listener() {
			...
			public void onOpen() {
				...
				YYDVideoServer.getInstance().WVMInvite(numberType, number, roomId, clientId, new CmdCallBacker() {
					public void onSuccess(Object arg) {
						log.d(TAG, "WVMInvite success");
					}
					
					public void onFailed(int error) {
						log.d(TAG, "WVMInvite failed, error: " + error);
					}
				});
			...
			public void onCancel() {
				...
				YYDVideoServer.getInstance().WVMInviteCancel(numberType, number, new CmdCallBacker() {
					public void onSuccess(Object arg) {
						log.d(TAG, "WVMInviteCancel success");
					}

					public void onFailed(int error) {
						log.d(TAG, "WVMInviteCancel failed, error: " + error);
					}
				});
			}
		...
		dialog.setTimeout(ActivityDialByContact_BAK.INVITE_TIMEOUT);
		dialog.show();
	}
}
