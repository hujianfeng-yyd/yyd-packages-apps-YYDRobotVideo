...
public class CircleImageView extends ImageView {
	...
	public void draw(Canvas canvas) {
		...
		float scale = (getWidth()) * 1.0f
				/ Math.min(bitmap.getWidth() - border - border, bitmap.getHeight() - border - border);
		mMatrix = new Matrix();
		...
		drawCircleBorder(canvas, mRadius + getPaddingLeft(), mRadius + getPaddingTop(),
				mRadius - getPaddingLeft() - getPaddingRight(), isSelected() ? mSelectedColor : mBorderColor);
		canvas.drawCircle(mRadius + getPaddingLeft(), mRadius + getPaddingTop(),
				mRadius - getPaddingLeft() - getPaddingRight() - border, paint);
	}
